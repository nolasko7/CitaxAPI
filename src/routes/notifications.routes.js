...
router.get("/", async (req, res) => {
  try {
    ...
    const params = afterId ? [companyId, afterId] : [companyId];
    ...
    const items = rows.map((row) => {
      ...
      return {
        ...
        createdAt: row.fecha_hora
          ? new Date(row.fecha_hora).toISOString()
    ...
    const [[unreadRow]] = await pool.execute(unreadQuery, [
      companyId,
      readMarker,
    ]);
...
router.patch("/read-all", async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ error: "Error al marcar notificaciones como leidas" });
  }
});
